...
class ColoredMNIST4(ColoredMNIST):
    ENVIRONMENTS = ['+90%', '+80%', '+70%', '-90%']

    def __init__(self, root, test_envs, hparams):
        super(ColoredMNIST, self).__init__(root, [0.1, 0.2, 0.3, 0.9],
                                         self.color_dataset, (2, 28, 28,), 2)
...
class SpuriousPACS(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ['art_painting-cartoon-0.0', 'art_painting-cartoon-1.0', 'photo']
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "PACS_V3/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
...
class MIDRCRace(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["Black_or_African_American", "White", "Other"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "race/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class MIDRCSex(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ["Male", "Female"]
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "sex/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)

class MIDRCState(MultipleEnvironmentImageFolder):
    CHECKPOINT_FREQ = 300
    ENVIRONMENTS = ['CA', 'IL', 'IN', 'NC', 'TC']
    def __init__(self, root, test_envs, hparams):
        self.dir = os.path.join(root, "state/")
        super().__init__(self.dir, test_envs, hparams['data_augmentation'], hparams)
