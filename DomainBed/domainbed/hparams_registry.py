...
def _hparams(algorithm, dataset, random_seed, ablat_TCRI_TCRI=False,
                ablat_TCRI_TIC=False):
    """
    ...
    if algorithm in ['DANN', 'CDANN']:
        ...
        _hparam('irm_penalty_anneal_ilters', 500,
    ...
    if dataset in SMALL_IMAGES:
        ...
        _hparam('batch_size', 8, lambda r: 8)
    elif "VLCS" in dataset:
        _hparam('batch_size', 4, lambda r: int(2**r.uniform(3, 5)))
    elif dataset in ['DomainNet', 'PACS', 'SpuriousPACS']:
        _hparam('batch_size', 4, lambda r: int(2**r.uniform(3, 5)))
    elif "midrc" in dataset.lower():
...
def default_hparams(algorithm, dataset, ablat_TCRI_TIC=False,
                ablat_TCRI_TCRI=False):
    return {a: b for a, (b, c) in _hparams(algorithm, dataset, 0,
                ablat_TCRI_TIC, ablat_TCRI_TCRI).items()}


def random_hparams(algorithm, dataset, seed, ablat_TCRI_TIC=False,
                ablat_TCRI_TCRI=False):
    return {a: c for a, (b, c) in _hparams(algorithm, dataset, seed,
                ablat_TCRI_TIC, ablat_TCRI_TCRI).items()}
