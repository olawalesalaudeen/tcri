...
def TIC_accuracy(network, loader, weights, device, i):
    correct = 0
    total = 0
    weights_offset = 0

    if i >= len(network.spurious_classifiers):
      return -1
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            F_x = network.featurizer(x)
            phi_dg = network.domain_general_featurizer(F_x)
            phi_spu = network.spurious_featurizer(F_x)

            latent_x = torch.cat([phi_dg, phi_spu], 1)

            p = network.spurious_classifiers[i](latent_x)
...
def tcri(network, loader, weights, device):
    all_tcri_val = 0
    total = 0
    weights_offset = 0
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            F_x = network.featurizer(x)
            phi_x = network.domain_general_featurizer(F_x)
            psi_x = network.spurious_featurizer(F_x)
            tcri_val = network.tcri(phi_x, psi_x, y)
            ...
            batch_weights = batch_weights.to(device)
            all_tcri_val += (tcri_val * batch_weights.view(-1, 1)).sum().item()

            total += batch_weights.sum().item()
    network.train()

    return all_tcri_val / total
